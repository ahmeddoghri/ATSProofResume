...
def generate_recommendations(
    ...
    try:
        ...
        prompt = _create_recommendations_prompt(job_description, resume_text)
        
        # Handle different model requirements
        try:
            logger.info(f"Generating recommendations using model: {model}")
            
            response = _get_model_response(client, model, prompt, temperature)
...
def _create_recommendations_prompt(job_description, resume_text):
    """
...
def _get_model_response(client, model, prompt, temperature):
    """
